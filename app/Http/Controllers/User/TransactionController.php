...
use App\{CourseUser, Transaction, Order, OrderItem, Databank, Admin, Cart, User, Taxa, Notification};
use Auth;
        ...
        if (!count($idUsers)) {
            ...
            foreach ($idUsers as $key => $idUser) {
                foreach ($auth->cart as $cart) {
                    $discount = number_format($cart->pivot->where('teacher_id', $idUser)->where('type', 2)->sum('discount'), 2, '', '');
                }
            ...
            foreach ($idUsers as $key => $idUser) {
                foreach ($auth->cart as $cart) {
                    $discount = $cart->pivot
                        ...
                        ->where('type', 2)->sum('discount');
                }
                $databank = Databank::where('user_id', $idUser)->first();

                $coursePrice = $auth->cart->where('user_id_owner', $idUser)->where('course_type', 2)->sum('price');
                $taxaUser = $taxa->taxa_users / 100;
                $totalSplit = ($coursePrice - $discount) * $taxaUser;
                $money = number_format($totalSplit, 2, '', '');
                $split_rules[$key] = ([
                    'amount' => $money,
                    'recipient_id' => $databank->recipient_id,
                    'charge_processing_fee' => true,
            ...
            foreach ($idAdmins as $key => $idAdmin) {
                $discount = Cart::where('teacher_id', $idAdmin)->where('type', 1)->where('user_id', $auth->id)->sum('discount');
                $money = 0;
                foreach ($split_rules as  $rules) {
                    $money += $rules['amount'];
                }
                // $discountSplit = number_format($discount, 2, '', '');
                $totalSplit = $request->pagarme['amount'] - $money;
                $split_rules[$key + $tmp] = ([
                    'amount' => $totalSplit,
                    'recipient_id' => config('services.pagarme.recipient_id'),
                    'charge_processing_fee' => true,
