        ...
        <div class="row">
            ...
            <div class="col-sm-8">
                ...
                <button class="btn btn-header act-test" data-course_id="{{$chapter->course->id}}"
                    data-chapter_id="{{$chapter->id}}">NOVA AVALIAÇÃO</button>
                <button class="btn btn-header act-class" data-course_id="{{$chapter->course->id}}"
                    data-chapter_id="{{$chapter->id}}">NOVA AULA</button>
            </div>
        </div>
    </section>
                    ...
                    <div class="box-body table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                ...
                                @if($item->course_item_type->id <= 6) <tr>
                                    <td>{{$item->name}}</td>
                                    ...
                                    <td>
                                        ...
                                        <a href="#"
                                            data-url="{{ route('teacher.course.item.test', ['id' => $item->id])}}"
                                            title="Incluir Perguntas" class="act-list test-index">
                                            <i class="fa fa-bars" aria-hidden="true"></i>
                                        </a>
                                        @endif
                                        @if($item->course_item_type->id != 6)
                                        @if($item->free_item === NULL)
                                        <a href="{{ route('teacher.course.item.free', ['id' => $item->id])}}"
                                            title="Aula Grátis" class="act-list">
                                        ...
                                        @endif
                                        @endif
                                        <a href="#" title="Editar" class="act-list act-edit-item"
                                            data-name="{{$item->name}}" data-desc="{{$item->desc}}"
                                            data-id="{{$item->id}}" data-file="{{$item->path}}"
                                            data-url="{{asset('uploads/archives/')}}"
